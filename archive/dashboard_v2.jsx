...
// ═══ P1 MONITORING ═══
function P1({mb,hist,alog,profileName,diagram:D,customSvg,T}){const S=mkS(T);const TT={contentStyle:{background:T.ttBg,border:`1px solid ${T.cardBorder}`,borderRadius:8,fontSize:11,color:T.text}};
...
return(<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gridTemplateRows:`${rowH} ${rowH}`,gap:10}}>
  ...
  <div style={crd}><div style={{...S.title,flexShrink:0}}><span>Alarmy</span><span style={{fontSize:10,color:T.textD}}>{alog.length}</span></div>
    <div style={{flex:1,minHeight:0,overflowY:"auto",fontSize:11}}>{alog.length===0?<div style={{color:T.textD,textAlign:"center",padding:20}}>Brak alarmów</div>:
...
function P3({sample,setSample,toast,addLog,sendCmd,T}){const S=mkS(T);
  const F=({label,k,ph,area})=>(<div style={S.box}><div style={S.lbl}>{label}</div>{area?<textarea value={sample[k]||""} onChange={e=>setSample(s=>({...s,[k]:e.target.value}))} placeholder={ph} rows={3} style={{...S.input,resize:"vertical"}}></textarea>:<input value={sample[k]||""} onChange={e=>setSample(s=>({...s,[k]:e.target.value}))} placeholder={ph} style={S.input}/>}</div>);
  return(<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
    ...
    <div style={S.card}><div style={S.title}><span>JSON → baza danych</span></div>
      ...
      <div style={{display:"flex",gap:6,marginTop:8}}>
        <button style={{...S.btn,background:"#0077b6"}} onClick={()=>{sendCmd?.("sample_info",sample);addLog(`Próbka ${sample.sampleId||"?"} zapisana`,"data");toast("Zapisano","success")}}>💾 Zapisz</button>
        ...
        <div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:11}}>
          ...
          <tbody>{Object.entries(users).map(([login,u],i)=>(<tr key={login} style={{borderBottom:`1px solid ${T.tblB}`,background:editUser===login?`${T.textA}11`:i%2?T.logAlt:"transparent"}}>
            ...
            <td style={{padding:"4px 6px"}}><div style={{display:"flex",gap:3}}>
              ...
              {login!=="admin"&&<button onClick={()=>delUser(login)} style={{background:"none",border:"none",color:"#ff4455",cursor:"pointer",fontSize:12}} title="Usuń">🗑</button>}</div></td>
          </tr>))}</tbody></table></div>
...
function P7({reports,setReports,sample,profileName,toast,addLog,sendCmd,T}){const S=mkS(T);
  ...
  const exportHTML=()=>{const html=genHTML();const b=new Blob([html],{type:"text/html"});const a=document.createElement("a");a.href=URL.createObjectURL(b);a.download=`raporty_${Date.now()}.html`;a.click();addLog("Eksport raportów HTML","export");toast("HTML OK","success")};
    ...
    <div style={{height:"100vh",display:"flex",flexDirection:"column",background:T.bg,fontFamily:"'Segoe UI',system-ui,sans-serif",color:T.text,overflow:"hidden"}}>
      ...
      <div style={{display:"flex",flex:1,minHeight:0,overflow:"hidden"}}>
        ...
        <main style={{flex:1,padding:12,minHeight:0,overflowY:"auto",overflowX:"hidden"}}>
          {ac===1&&<P1 mb={mb} hist={hist} alog={alog} profileName={profileName} diagram={diagram} customSvg={customSvg} T={T}/>}
          ...
          {ac===7&&<P7 reports={reports} setReports={setReports} sample={sample} profileName={profileName} toast={toast} addLog={addLog} sendCmd={sendCmd} T={T}/>}
        </main></div>
      <Footer T={T}/>
